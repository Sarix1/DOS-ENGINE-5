...
void drawObject(Object_t* obj)
{
    ...
    {
        vecs2scaleRotV(obj->transformed_poly.points,
                       obj->poly->points,
                       obj->transformed_poly.num_points,
                       obj->scale,
                       obj->dir);
    ...
    if (obj->flags & OBJ_FLAG_FILLED)
    {
        drawShapeFilled(obj->pos,
                        obj->transformed_poly.points,
                        obj->transformed_poly.num_points,
                        obj->color);
    }
    else
...
void drawAllObjects()
{
    ...
    drawLineFixp(av2(PLAYER_OBJ->pos),
                 av2(vec2add( PLAYER_OBJ->pos, vec2scale(PLAYER_OBJ->dir,F(1000)))),
                 COLOR_GREEN);
}
