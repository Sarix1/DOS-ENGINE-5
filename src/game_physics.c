...
{
    switch (obj->controldata & (BIT_ACTION_LEFT|BIT_ACTION_RIGHT))
    {
        case BIT_ACTION_LEFT:
            obj->angvel = PHYS_TURN_RATE;
            break;
        case BIT_ACTION_RIGHT:
            obj->angvel = -PHYS_TURN_RATE;
            break;
        default:
            obj->angvel = 0;
    ...
    switch (obj->controldata & (BIT_ACTION_UP|BIT_ACTION_DOWN))
    {
        case BIT_ACTION_UP:
            obj->vel = vec2add(obj->vel, vec2scale(obj->dir, PHYS_ACCELERATE));
            break;
        case BIT_ACTION_DOWN:
            obj->vel = vec2add(obj->vel, vec2scale(obj->dir, -PHYS_ACCELERATE));
            break;
        default:
